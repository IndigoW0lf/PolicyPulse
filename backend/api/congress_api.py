...
import logging
from backend.utils import keywords
...
# GET bills from endpoint
def fetch_bill_summary(congress, bill_type, bill_number):
    manage_api_state(api_state, 1)
    endpoint = f"v3/bill/{congress}/{bill_type}/{bill_number}/summaries"
    logging.debug(f"Fetching summary for bill: {congress}/{bill_type}/{bill_number}")
    return make_request(endpoint, api_state=api_state)

# Run bills through keyword analysis
...
def store_bill(data, batch_size=50):
    ...
    for item in data:
        ...
        # Fetch the bill summary first
        congress = item.get('congress')
        bill_type = item.get('type')
        bill_number = item.get('number')
        
        summary_data = fetch_bill_summary(congress, bill_type, bill_number)
        
        # If there's a valid summary, analyze it
        if summary_data:
            summary_text = summary_data.get('summaryText')  # Adjust key if different in the API response
            is_relevant = keyword_analysis(summary_text)
            
            # If the bill is relevant based on the summary, save or tag it as necessary
            if is_relevant:
