...
def test_loc_summary_creation(loc_summary_factory, bill_factory):
    ...
    loc_summary = loc_summary_factory(bill=bill, versions=[{
    ...
    assert loc_summary.versions[0]['version_code'] == "Introduced"
    assert loc_summary.versions[0]['chamber'] == "House"
    assert loc_summary.versions[0]['action_description'] == "Introduced in House"
    assert loc_summary.versions[0]['summary_text'] == "This is a test summary"

    # Fetch the loc_summary from the database and check the fields
    fetched_loc_summary = LOCSummary.query.get(loc_summary.id)
    assert fetched_loc_summary.versions[0]['version_code'] == loc_summary.versions[0]['version_code']
    assert fetched_loc_summary.versions[0]['chamber'] == loc_summary.versions[0]['chamber']
    assert fetched_loc_summary.versions[0]['action_description'] == loc_summary.versions[0]['action_description']
    assert fetched_loc_summary.versions[0]['summary_text'] == loc_summary.versions[0]['summary_text']
...
def test_loc_summary_field_validations(session, loc_summary_factory):
    ...
    with pytest.raises(IntegrityError):
        loc_summary = loc_summary_factory(versions=None)
        session.add(loc_summary)
        session.commit()
...
def test_loc_summary_relationships(loc_summary_factory, bill_factory):
    ...
    bill = bill_factory(title="Test Bill")
    loc_summary = loc_summary_factory(bill=bill, versions=[{"version_code": "Introduced"}])
